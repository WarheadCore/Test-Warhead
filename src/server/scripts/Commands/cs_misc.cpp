...
class misc_commandscript : public CommandScript
{
    public:
        ...
        static bool HandleGPSCommand(ChatHandler* handler, char const* args)
        {
            ...
            ZLiquidStatus status = map->GetLiquidStatus(object->GetPhaseMask(), object->GetPositionX(), object->GetPositionY(), object->GetPositionZ(), MAP_ALL_LIQUIDS, &liquidStatus,
        ...
        **/
        static bool HandlePInfoCommand(ChatHandler* handler, char const* args)
        {
            ...
            if (banTime >= 0)
                handler->PSendSysMessage(LANG_PINFO_BANNED, banType.c_str(), banReason.c_str(), banTime > 0 ? secsToTimeString(banTime - GameTime::GetGameTime(),
                                         TimeFormat::ShortText).c_str() : handler->GetWarheadString(LANG_PERMANENTLY), bannedBy.c_str());
            ...
            handler->PSendSysMessage(LANG_PINFO_CHR_RACE, (gender == 0 ? handler->GetWarheadString(LANG_CHARACTER_GENDER_MALE) : handler->GetWarheadString(LANG_CHARACTER_GENDER_FEMALE)), raceStr.c_str(),
                                     classStr.c_str());
