...
enum ICCMisc { GOSSIP_MENUID_ALLY = 11204, GOSSIP_MENUID_HORDE = 11207 };
  ...
  struct npc_highlord_tirion_fordringAI : public ScriptedAI {
    ...
    void SetData(uint32 type, uint32 data) override {
      ...
      if (Creature *theLichKing =
              me->FindNearestCreature(NPC_THE_LICH_KING_LH, 150.0f)) {
        if (Creature *bolvarFordragon = me->FindNearestCreature(
          ...
          if (Creature *factionNPC = me->FindNearestCreature(
                  _instance->GetData(DATA_TEAM_IN_INSTANCE) == HORDE
    ...
    void UpdateAI(uint32 diff) override {
      ...
      while (uint32 eventId = _events.ExecuteEvent()) {
        ...
        case EVENT_BOLVAR_INTRO_1:
          if (Creature *bolvarFordragon =
                  ObjectAccessor::GetCreature(*me, _bolvarFordragon)) {
...
public:
  ...
  bool operator()(Unit *unit) const {
    ...
    if (unit->HasAura(SPELL_DEATH_PLAGUE_AURA) ||
        unit->HasAura(SPELL_RECENTLY_INFECTED))
      return false;
...
public:
  ...
  struct npc_alchemist_adriannaAI : public ScriptedAI {
    ...
    bool GossipHello(Player *player) override {
      if (!me->FindCurrentSpellBySpellId(sSpellMgr->GetSpellIdForDifficulty(
              SPELL_HARVEST_BLIGHT_SPECIMEN, me)))
...
public:
  ICCOrbControllerMinionSearch(Unit *owner, bool checkCasting)
      : _owner(owner), _checkCasting(checkCasting) {}

  bool operator()(Creature *target) const {
    if (!target->IsAlive() ||
        (_checkCasting && target->HasUnitState(UNIT_STATE_CASTING)) ||
        target->GetWaypointPath() || _owner->GetDistance(target) > 10.0f)
      return false;
...
static Emote const DarkFallensEmotes[] = {
    EMOTE_ONESHOT_TALK,  EMOTE_ONESHOT_EXCLAMATION, EMOTE_ONESHOT_QUESTION,
    EMOTE_ONESHOT_LAUGH, EMOTE_ONESHOT_YES,         EMOTE_ONESHOT_NO};

struct npc_icc_orb_controller : public ScriptedAI {
  npc_icc_orb_controller(Creature *creature)
      : ScriptedAI(creature), _isInCombat(false), _isLongRepeat(false) {}

  void Reset() override {
    _scheduler.Schedule(1s, [this](TaskContext /*initialize*/) {
      std::vector<Creature *> creatures;
      ICCOrbControllerMinionSearch check(me, false);
      Warhead::CreatureListSearcher<ICCOrbControllerMinionSearch> searcher(
          me, creatures, check);
  ...
  void ScheduleVisualChannel(bool evading) {
    _scheduler.Schedule(evading ? 5s : 1s, [this](TaskContext visual) {
      ObjectGuid guid =
          Warhead::Containers::SelectRandomContainerElement(_minionGuids);
  ...
  void UpdateValidGuids() {
    for (GuidVector::iterator itr = _minionGuids.begin();
         itr != _minionGuids.end();) {
  ...
  void SpellHit(WorldObject *caster, SpellInfo const *spellInfo) override {
    if (spellInfo->Id == SPELL_ORB_CONTROLLER_ACTIVE)
      if (GameObject *orb =
              me->FindNearestGameObject(GO_EMPOWERING_BLOOD_ORB, 5.0f))
  ...
  void SetGUID(ObjectGuid const &guid, int32 id) override {
    ...
    if (Unit *minion = ObjectAccessor::GetUnit(
            *me,
            Warhead::Containers::SelectRandomContainerElement(_minionGuids)))
...
struct DarkFallenAI : public ScriptedAI {
  DarkFallenAI(Creature *creature)
      : ScriptedAI(creature), IsDoingEmotes(true), AttackSpellId(0) {}
    ...
    Scheduler
        ...
        .Schedule(1s, 10s, [this](TaskContext emote) {
          ...
          if (roll_chance_i(20)) {
            ...
            Warhead::CreatureListSearcher<ICCOrbControllerMinionSearch>
                searcher(me, creatures, check);
            Cell::VisitGridObjects(me, searcher, 10.0f);
            if (!creatures.empty()) {
              Creature *friendly =
                  Warhead::Containers::SelectRandomContainerElement(creatures);
              DoCast(friendly, SPELL_POLYMORPH_ALLY);
            }
          }
          Scheduler.Schedule(1s, [this](TaskContext /*emote*/) {
            me->HandleEmoteCommand(
                Warhead::Containers::SelectRandomContainerElement(
                    DarkFallensEmotes));
          });
  ...
  void DoAction(int32 action) override {
    if (action == ACTION_SIPHON_INTERRUPTED)
      if (GameObject *orb =
              me->FindNearestGameObject(GO_EMPOWERING_BLOOD_ORB, 10.0f))
  ...
  void ScheduleSpells() override {
    ...
    Scheduler
        .Schedule(500ms,
                  [this](TaskContext /*context*/) {
                    if (Unit *target =
                            SelectTarget(SelectTargetMethod::Random, 0, 0.0f,
                                         true, false, -SPELL_CHAINS_OF_SHADOW))
                      DoCast(target, SPELL_CHAINS_OF_SHADOW);
                  })

        .Schedule(11s, [this](TaskContext summonVampiric) {
          // Vampiric should be summoned by 70647 but i have no idea what is
          // miscB of summon effect
          if (Unit *target = me->GetVictim())
            if (Creature *vampiric = me->SummonCreature(
                    NPC_VAMPIRIC_FIEND, target->GetPosition(),
...
struct go_empowering_blood_orb : public GameObjectAI {
  ...
  void Reset() override {
    if (Creature *trigger =
            me->SummonCreature(NPC_ORB_VISUAL_STALKER, me->GetPosition(),
  ...
  void HandleObjectUse() {
    ...
    _scheduler.Schedule(3s, [this](TaskContext /*context*/) { me->Delete(); });
  }
...
class spell_icc_empowered_blood_3 : public AuraScript {
  ...
  void Register() override {
    OnEffectApply += AuraEffectApplyFn(
        spell_icc_empowered_blood_3::OnApply, EFFECT_0,
        SPELL_AURA_MOD_DAMAGE_PERCENT_DONE, AURA_EFFECT_HANDLE_REAL);
    AfterEffectRemove += AuraEffectRemoveFn(
        spell_icc_empowered_blood_3::OnRemove, EFFECT_0,
        SPELL_AURA_MOD_DAMAGE_PERCENT_DONE, AURA_EFFECT_HANDLE_REAL);
  }
};
...
class spell_icc_siphon_essence : public AuraScript {
  ...
  void Register() override {
    AfterEffectRemove +=
        AuraEffectRemoveFn(spell_icc_siphon_essence::OnRemove, EFFECT_1,
                           SPELL_AURA_MOD_ROOT, AURA_EFFECT_HANDLE_REAL);
  }
};
...
class spell_darkfallen_blood_mirror : public SpellScript {
  ...
  bool Validate(SpellInfo const * /*spell*/) override {
    return ValidateSpellInfo(
        {SPELL_BLOOD_MIRROR_2, SPELL_BLOOD_MIRROR_DAMAGE_SHARE});
  }
  ...
  void Register() override {
    OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
        spell_darkfallen_blood_mirror::FilterTargets, EFFECT_0,
        TARGET_UNIT_SRC_AREA_ENEMY);
    OnEffectLaunch += SpellEffectFn(spell_darkfallen_blood_mirror::HandleMirror,
...
class spell_generic_remove_empowered_blood : public SpellScript {
  ...
  void Register() override {
    OnEffectHitTarget +=
        SpellEffectFn(spell_generic_remove_empowered_blood::HandleScript,
...
public:
  ...
  class spell_icc_stoneform_AuraScript : public AuraScript {
    ...
    void Register() override {
      OnEffectApply +=
          AuraEffectApplyFn(spell_icc_stoneform_AuraScript::OnApply, EFFECT_0,
                            SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
      OnEffectRemove +=
          AuraEffectRemoveFn(spell_icc_stoneform_AuraScript::OnRemove, EFFECT_0,
                             SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
    }
  };
...
public:
  ...
  class spell_icc_sprit_alarm_SpellScript : public SpellScript {
    ...
    void HandleEvent(SpellEffIndex effIndex) {
      ...
      if (GameObject *trap =
              GetGObjCaster()->FindNearestGameObject(trapId, 5.0f))
        trap->SetRespawnTime(trap->GetGOInfo()->GetAutoCloseTime() /
                             IN_MILLISECONDS);

      std::list<Creature *> wards;
      GetGObjCaster()->GetCreatureListWithEntryInGrid(
          wards, NPC_DEATHBOUND_WARD, range);
      ...
      for (auto const &itr : wards) {
        ...
        if (deathboundWard->IsAlive() &&
            deathboundWard->HasAura(SPELL_STONEFORM)) {
    ...
    void Register() override {
      OnEffectHit +=
          SpellEffectFn(spell_icc_sprit_alarm_SpellScript::HandleEvent,
                        EFFECT_2, SPELL_EFFECT_SEND_EVENT);
    }
  };
...
class spell_frost_giant_death_plague : public SpellScript {
  ...
  bool Validate(SpellInfo const * /*spell*/) override {
    return ValidateSpellInfo(
        {SPELL_RECENTLY_INFECTED, SPELL_DEATH_PLAGUE_KILL, SPELL_DEATH_PLAGUE});
  }
  ...
  void FilterTargets(std::list<WorldObject *> &targets) {
    ...
    targets.remove_if([](WorldObject *obj) -> bool {
      ...
      if (object->HasAura(SPELL_RECENTLY_INFECTED) ||
          object->HasAura(SPELL_DEATH_PLAGUE_AURA))
        return true;
  ...
  void Register() override {
    OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
        spell_frost_giant_death_plague::CountTargets, EFFECT_0,
        TARGET_UNIT_SRC_AREA_ALLY);
    OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
        spell_frost_giant_death_plague::FilterTargets, EFFECT_1,
        TARGET_UNIT_SRC_AREA_ALLY);
    OnEffectHitTarget +=
        SpellEffectFn(spell_frost_giant_death_plague::HandleScript, EFFECT_1,
  ...
  class spell_icc_harvest_blight_specimen_SpellScript : public SpellScript {
    ...
    void Register() override {
      OnEffectHitTarget += SpellEffectFn(
          spell_icc_harvest_blight_specimen_SpellScript::HandleScript, EFFECT_0,
          SPELL_EFFECT_SCRIPT_EFFECT);
      OnEffectHitTarget += SpellEffectFn(
          spell_icc_harvest_blight_specimen_SpellScript::HandleQuestComplete,
          EFFECT_1, SPELL_EFFECT_QUEST_COMPLETE);
    }
  };
  ...
  class spell_icc_soul_missile_SpellScript : public SpellScript {
    ...
    void Register() override {
      OnDestinationTargetSelect += SpellDestinationTargetSelectFn(
          spell_icc_soul_missile_SpellScript::RelocateDest, EFFECT_0,
          TARGET_DEST_CASTER);
    }
  };
...
spell_trigger_spell_from_caster::spell_trigger_spell_from_caster(
    uint32 triggerId, TriggerCastFlags triggerFlags /*= TRIGGERED_FULL_MASK*/)
...
public:
  ...
  bool OnTrigger(Player *player,
                 AreaTriggerEntry const * /*areaTrigger*/) override {
    ...
    player->TeleportTo(631, destPos.GetPositionX(), destPos.GetPositionY(),
                       destPos.GetPositionZ(), destPos.GetOrientation());
...
public:
  ...
  struct npc_icc_nerubar_broodkeeperAI : public ScriptedAI {
    ...
    void JustEnteredCombat(Unit * /*who*/) override { me->CallForHelp(15.0f); }
    ...
    void JustReachedHome() override {
      ...
      SetGravity();
      me->NearTeleportTo(me->GetPositionX(), me->GetPositionY(),
    ...
    void UpdateAI(uint32 diff) override {
      ...
      case 3:
        if (Unit *target =
                SelectTarget(SelectTargetMethod::Random, 0, 40.0f, true))
