...
enum MiscInfo {
  ...
  // Manual marking for targets hit by Bone Slice as no aura exists for this
  // purpose These units are the tanks in this encounter and should be immune to
...
public:
  ...
  struct boss_lord_marrowgarAI : public BossAI {
    ...
    void Reset() override {
      ...
      events.ScheduleEvent(EVENT_BONE_SPIKE_GRAVEYARD, 10s, 15s,
                           EVENT_GROUP_SPECIAL);
    ...
    void UpdateAI(uint32 diff) override {
      ...
      while (uint32 eventId = events.ExecuteEvent()) {
        ...
        case EVENT_BONE_SPIKE_GRAVEYARD:
          ...
          events.ScheduleEvent(EVENT_BONE_SPIKE_GRAVEYARD, 15s, 20s,
                               EVENT_GROUP_SPECIAL);
          break;
        ...
        case EVENT_BONE_STORM_MOVE: {
          ...
          if (unit)
            me->GetMotionMaster()->MovePoint(POINT_TARGET_BONESTORM_PLAYER,
                                             *unit);
          break;
        }
        case EVENT_BONE_STORM_END:
          if (MovementGenerator *movement =
                  me->GetMotionMaster()->GetMovementGenerator(
                      [](MovementGenerator const *a) -> bool {
                        if (a->GetMovementGeneratorType() ==
                            POINT_MOTION_TYPE) {
                          PointMovementGenerator<Creature> const
                              *pointMovement = dynamic_cast<
                                  PointMovementGenerator<Creature> const *>(a);
          ...
          events.ScheduleEvent(EVENT_ENABLE_BONE_SLICE, 10s);
          if (!IsHeroic())
            events.RescheduleEvent(EVENT_BONE_SPIKE_GRAVEYARD, 15s, 20s,
                                   EVENT_GROUP_SPECIAL);
          break;
  ...
  struct npc_coldflameAI : public ScriptedAI {
    ...
    void IsSummonedBy(WorldObject *ownerWO) override {
      ...
      if (owner->HasAura(SPELL_BONE_STORM)) {
        ...
        owner->GetNearPoint2D(nullptr, pos.m_positionX, pos.m_positionY,
                              5.0f - owner->GetCombatReach(), ang);
      } else {
        Player *target = ObjectAccessor::GetPlayer(
            *owner, owner->GetAI()->GetGUID(DATA_COLDFLAME_GUID));
        if (!target) {
        ...
        me->SetOrientation(ang);
        owner->GetNearPoint2D(nullptr, pos.m_positionX, pos.m_positionY,
                              15.0f - owner->GetCombatReach(), ang);
      }
...
public:
  ...
  struct npc_bone_spikeAI : public ScriptedAI {
    npc_bone_spikeAI(Creature *creature)
        : ScriptedAI(creature), _hasTrappedUnit(false) {
    ...
    void PassengerBoarded(Unit *passenger, int8 /*seat*/, bool apply) override {
      ...
      passenger->GetMotionMaster()->LaunchMoveSpline(
          std::move(init), EVENT_VEHICLE_BOARD, MOTION_PRIORITY_HIGHEST);
    }
  ...
  class spell_marrowgar_coldflame_SpellScript : public SpellScript {
    ...
    void SelectTarget(std::list<WorldObject *> &targets) {
      ...
      Unit *target = GetCaster()->GetAI()->SelectTarget(
          SelectTargetMethod::Random, 0, -GetCaster()->GetCombatReach(), true,
          false, -SPELL_IMPALED);
      if (!target)
        target = GetCaster()->GetAI()->SelectTarget(
            SelectTargetMethod::Random, 0, 0.0f,
            true); // or the tank if its solo
      if (!target)
        return;
    ...
    void Register() override {
      OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
          spell_marrowgar_coldflame_SpellScript::SelectTarget, EFFECT_0,
          TARGET_UNIT_DEST_AREA_ENEMY);
      OnEffectHitTarget += SpellEffectFn(
          spell_marrowgar_coldflame_SpellScript::HandleScriptEffect, EFFECT_0,
...
public:
  ...
  class spell_marrowgar_coldflame_SpellScript : public SpellScript {
    ...
    void HandleScriptEffect(SpellEffIndex effIndex) {
      PreventHitDefaultEffect(effIndex);
      for (uint8 i = 0; i < 4; ++i)
        GetCaster()->CastSpell(GetHitUnit(), uint32(GetEffectValue() + i),
                               true);
    }
...
public:
  ...
  class spell_marrowgar_coldflame_damage_AuraScript : public AuraScript {
    ...
    bool CanBeAppliedOn(Unit *target) {
      ...
      if (target->GetExactDist2d(GetOwner()) >
    ...
    void Register() override {
      DoCheckAreaTarget += AuraCheckAreaTargetFn(
          spell_marrowgar_coldflame_damage_AuraScript::CanBeAppliedOn);
    }
  };
  ...
  class spell_marrowgar_bone_spike_graveyard_SpellScript : public SpellScript {
    ...
    bool Load() override {
      return GetCaster()->GetTypeId() == TYPEID_UNIT &&
             GetCaster()->IsAIEnabled();
    }

    SpellCastResult CheckCast() {
      return GetCaster()->GetAI()->SelectTarget(
                 SelectTargetMethod::Random, 0,
                 BoneSpikeTargetSelector(GetCaster()->GetAI()))
                 ? SPELL_CAST_OK
    ...
    void HandleSpikes(SpellEffIndex effIndex) {
      ...
      if (Creature *marrowgar = GetCaster()->ToCreature()) {
        ...
        marrowgarAI->SelectTargetList(targets, boneSpikeCount,
                                      SelectTargetMethod::Random, 1,
                                      BoneSpikeTargetSelector(marrowgarAI));
        if (targets.empty())
          return;
        ...
        for (std::list<Unit *>::const_iterator itr = targets.begin();
          ...
          if (!target->IsAlive()) // make sure we don't get any stuck spikes on
                                  // dead targets
          {
            if (Aura *aura = target->GetAura(SPELL_IMPALED)) {
              if (Creature *spike = ObjectAccessor::GetCreature(
                      *target, aura->GetCasterGUID()))
    ...
    void Register() override {
      OnCheckCast += SpellCheckCastFn(
          spell_marrowgar_bone_spike_graveyard_SpellScript::CheckCast);
      OnEffectHitTarget += SpellEffectFn(
...
public:
  ...
  class spell_marrowgar_bone_storm_SpellScript : public SpellScript {
    ...
    void Register() override {
      OnHit +=
          SpellHitFn(spell_marrowgar_bone_storm_SpellScript::RecalculateDamage);
    }
  };
...
public:
  ...
  public:
    spell_marrowgar_bone_slice_SpellScript() { _targetCount = 0; }
  ...
  private:
    ...
    void CountTargets(std::list<WorldObject *> &targets) {
      _targetCount =
          std::min<uint32>(targets.size(), GetSpellInfo()->MaxAffectedTargets);
    }
    ...
    void Register() override {
      BeforeCast += SpellCastFn(
          spell_marrowgar_bone_slice_SpellScript::ClearSpikeImmunities);
      OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
          spell_marrowgar_bone_slice_SpellScript::CountTargets, EFFECT_0,
          TARGET_UNIT_DEST_AREA_ENEMY);
...
public:
  at_lord_marrowgar_entrance()
      : OnlyOnceAreaTriggerScript("at_lord_marrowgar_entrance") {}

  bool _OnTrigger(Player *player,
                  AreaTriggerEntry const * /*areaTrigger*/) override {
    if (InstanceScript *instance = player->GetInstanceScript())
      if (Creature *lordMarrowgar = ObjectAccessor::GetCreature(
              *player, instance->GetGuidData(DATA_LORD_MARROWGAR)))
