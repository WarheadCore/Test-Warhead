...
void LoginDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(LOGIN_SEL_IP_INFO_BY_ACC_ID, "SELECT ip_banned.bandate, ip_banned.unbandate, ip_banned.bannedby, ip_banned.banreason FROM ip_banned, account WHERE account.id = ? AND account.last_ip = ip_banned.ip AND (bandate = unbandate OR unbandate > UNIX_TIMESTAMP()) ORDER BY unbandate", CONNECTION_SYNCH);
    ...
    PrepareStatement(LOGIN_SEL_ACCOUNT_BANNED_INFO, "SELECT `bandate`, `unbandate`, `bannedby`, `banreason` FROM `account_banned` WHERE `id` = ? AND `active` = 1 AND UNIX_TIMESTAMP() <= `unbandate` ORDER BY `unbandate` DESC LIMIT 1;", CONNECTION_SYNCH);
