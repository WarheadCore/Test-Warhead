    ...
    do
    {
        ...
        if (itemId)
        {
            if (!sObjectMgr->GetItemTemplate(itemId))
                LOG_ERROR("mailexternal", "External Mail> Item entry %u from `mail_external` doesn't exist in DB, skipped.", itemId);
            else
            {
                if (Item* mailItem = Item::CreateItem(itemId, itemCount))
                {
                    ...
                    LOG_INFO("mailexternal", "External Mail> Adding %u of item with id %u for player_id %u", itemCount, itemId, receiver_guid);
                }
            }
        }
