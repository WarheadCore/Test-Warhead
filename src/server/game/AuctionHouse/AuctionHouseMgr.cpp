...
void AuctionHouseMgr::SendAuctionWonMail(AuctionEntry* auction, CharacterDatabaseTransaction trans)
{
    ...
    if ((bidder || bidderAccId) && !sAuctionBotConfig->IsBotChar(auction->bidder))
    {
        ...
        sMailMgr->SendMailByAuctionHouseWithItems(auction, auction->bidder, auction->BuildAuctionMailSubject(AUCTION_WON), AuctionEntry::BuildAuctionMailBody(auction->owner, auction->bid, auction->buyout, 0,
                0), 0, itemlist, MAIL_CHECK_MASK_COPIED);
        itemlist.clear();
    }
    else
    {
...
void AuctionHouseMgr::SendAuctionSalePendingMail(AuctionEntry* auction)
{
    ...
    if ((owner || owner_accId) && !sAuctionBotConfig->IsBotChar(auction->owner))
        sMailMgr->SendMailByAuctionHouse(auction, auction->owner, auction->BuildAuctionMailSubject(AUCTION_SALE_PENDING), AuctionEntry::BuildAuctionMailBody(auction->bidder, auction->bid, auction->buyout,
                                         auction->deposit, auction->GetAuctionCut()), 0, MAIL_CHECK_MASK_COPIED);
}
...
void AuctionHouseMgr::SendAuctionSuccessfulMail(AuctionEntry* auction)
{
    ...
    if ((owner || owner_accId) && !sAuctionBotConfig->IsBotChar(auction->owner))
    {
        ...
        sMailMgr->SendMailByAuctionHouse(auction, auction->owner, auction->BuildAuctionMailSubject(AUCTION_SUCCESSFUL), AuctionEntry::BuildAuctionMailBody(auction->bidder, auction->bid, auction->buyout,
                                         auction->deposit, auction->GetAuctionCut()), profit, MAIL_CHECK_MASK_COPIED, CONF_GET_INT("MailDeliveryDelay"));
    }
}
...
void AuctionHouseMgr::SendAuctionExpiredMail(AuctionEntry* auction, CharacterDatabaseTransaction trans)
{
    ...
    if ((owner || owner_accId) && !sAuctionBotConfig->IsBotChar(auction->owner))
    {
        ...
        std::list<Item*> itemlist;
        itemlist.push_back(pItem);
        sMailMgr->SendMailByAuctionHouseWithItems(auction, auction->owner, auction->BuildAuctionMailSubject(AUCTION_EXPIRED), AuctionEntry::BuildAuctionMailBody(0, 0, auction->buyout, auction->deposit, 0), 0,
                itemlist, MAIL_CHECK_MASK_COPIED);
        itemlist.clear();
    }
    else
    {
...
void AuctionHouseMgr::SendAuctionOutbiddedMail(AuctionEntry* auction, uint32 newPrice, Player* newBidder)
{
    ...
    if ((oldBidder || oldBidder_accId) && !sAuctionBotConfig->IsBotChar(auction->bidder))
    {
        ...
        sMailMgr->SendMailByAuctionHouse(auction, auction->bidder, auction->BuildAuctionMailSubject(AUCTION_OUTBIDDED), AuctionEntry::BuildAuctionMailBody(auction->owner, auction->bid, auction->buyout,
                                         auction->deposit, auction->GetAuctionCut()), auction->bid, MAIL_CHECK_MASK_COPIED);
    }
}
...
void AuctionHouseMgr::SendAuctionCancelledToBidderMail(AuctionEntry* auction)
{
    ...
    if ((bidder || bidder_accId) && !sAuctionBotConfig->IsBotChar(auction->bidder))
        sMailMgr->SendMailByAuctionHouse(auction, auction->bidder, auction->BuildAuctionMailSubject(AUCTION_CANCELLED_TO_BIDDER), AuctionEntry::BuildAuctionMailBody(auction->owner, auction->bid,
                                         auction->buyout, auction->deposit, 0), auction->bid, MAIL_CHECK_MASK_COPIED);
}
