...
void WorldSession::HandleAuctionRemoveItem(WorldPacket& recvData)
{
    ...
    if (auction && auction->owner == player->GetGUID().GetCounter())
    {
        ...
        if (pItem)
        {
            ...
            sMailMgr->SendMailByAuctionHouseWithItems(auction, player->GetGUID().GetCounter(), auction->BuildAuctionMailSubject(AUCTION_CANCELED), AuctionEntry::BuildAuctionMailBody(0, 0, auction->buyout,
